...
void decode(const char * fileIn, const char *sortType);
...
// This function use encoded txt to decode it back
// to english text
void decode(const char *fileIn, const char *sortType)
{
  ...
  // skipping the initial n+2 lines of the
  // encoded text input
  int totalChars = numOfSymbols+2;
  char currChar;
  struct symbol *curr = root;
  while (totalChars > 0)
  {
    cin.ignore(1000,'\n');
    totalChars--;
  ...
  // decoding the ecoded text and sending it to
  // standard output.
  while (!cin.eof())
  {
    cin >> currChar;
